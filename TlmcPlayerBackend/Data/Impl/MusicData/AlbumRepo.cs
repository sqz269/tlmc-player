...
public class AlbumRepo : IAlbumRepo
{
    ...
    public async Task<IEnumerable<Album>> GetAlbumsFiltered(AlbumFilter filter, int start, int limit)
    {
        ...
        {
            query = query.Where(a => Regex.IsMatch(a.Name.Default, filter.Title, RegexOptions.IgnoreCase));
        }
        ...
        if (filter is { Artist: not null, ArtistId: null })
        {
            ...
            query = circle switch
            {
                null => throw new KeyNotFoundException($"Invalid Artist '{filter.Artist}'"),
                _ => query.Where(a => a.AlbumArtist.Contains(circle))
            };
        }
        ...
        if (filter.ArtistId != null)
        {
            ...
            query = circle switch
            {
                null => throw new KeyNotFoundException($"Invalid Artist Id: '{filter.ArtistId}"),
                _ => query.Where(a => a.AlbumArtist.Contains(circle))
            };
        }
